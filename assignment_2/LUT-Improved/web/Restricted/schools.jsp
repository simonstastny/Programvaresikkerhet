...
<sql:query var="country_name" dataSource="jdbc/lut2">
...
<html>
    ...
    <body>
        <c:set var="country" value="${country_name.rows[0]}"/>
        <c:choose>
            <c:when test="${ empty country }">
               <h1>According to our database, there are no approved schools in ${param.country}</h1>
            </c:when>
            <c:otherwise>
